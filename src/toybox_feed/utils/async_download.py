    ...
    async def fetch_file(
        ...
        async with limiter:
            ...
            async with session.get(url) as response:
                logger.info(f"Recieved response {response.status}")
    ...
    async def main() -> None:
        ...
        async with aiohttp.ClientSession(headers=headers, **kwargs) as session:
            await asyncio.gather(
                *[fetch_file(session, url, dir) for url in urls]  # type: ignore - always will be Path type
            )
...
if __name__ == "__main__":
    ...
    start_time = timer()
    download_many(urls, "./torrents-files-async2")
    logger.info(
